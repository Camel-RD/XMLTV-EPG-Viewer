...
namespace xmltv
{
    public partial class UCEPGView2 : UserControl, IEPGView
    {
        ...
        void FirstSelect()
        {
            if (cbChannels.Items.Count == 0) return;
        ...
        void RefreshData()
        {
            ...
            cbChannels.Items.Clear();
            foreach(string nm in _topManager.ChannelsByGroup.Keys)
            {
                cbChannels.Items.Add(nm);
            }
        ...
        void RefreshProgrammList(int dtnr, int chnr)
        {
            ...
            string groupname = cbChannels.Text;
            DateTime dt = _topManager.UsedDates[dtnr];

            if (_topManager.FilterWithTags)
            {
                ProgrammList = _topManager.EPGData.GetByDateAndGroupCheckTag(dt, groupname);
            }
            else
            {
                ProgrammList = _topManager.EPGData.GetByDateAndGroup(dt, groupname);
            }
